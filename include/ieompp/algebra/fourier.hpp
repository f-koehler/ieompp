#ifndef IEOMPP_ALGEBRA_FOURIER_HPP_
#define IEOMPP_ALGEBRA_FOURIER_HPP_
    ...
    namespace algebra
    {
        ...
        struct Transformation {
            ...
            void transform(const Term& term, TermList<TermFourier>& result)
            {
                ...
                for(const auto& idx : disc_a) {
                    auto factor = term.prefactor
                                  * std::exp(exponent * dot_product(disc_a[op.index], disc_b[idx]));
                    result.push_back(
