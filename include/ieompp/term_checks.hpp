#ifndef IEOMPP_TERM_CHECKS_HPP_
#define IEOMPP_TERM_CHECKS_HPP_
...
namespace ieompp
{
    ...
    {
        static_assert(
            std::is_same<typename index_type<Index, typename Term::Operator>::type, bool>::value,
            "spin index must be a boolean");

        int spin = 0;
        for(auto& op : t.operators) {
            if(op.creator) spin += get_index<Index>(op) ? 1 : -1;
            else spin -= get_index<Index>(op) ? 1 : -1;
        }
        return spin;
    }
