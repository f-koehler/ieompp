...
{
    Application::name        = "hubbard_real_1d_rk4_jump_no";
    ...
    // setup observable
    using Operator = typename decltype(basis)::Monomial::Operator;
    const auto expectation_values = hubbard::ExpectationValue1DHalfFilled<double, Lattice>{lattice};
    const auto expectation_value_function = [&expectation_values](
        const Operator& a, const Operator& b) { return expectation_values(a, b); };
    const auto fermi_jump =
        hubbard::FermiJump1D_NO<double, Basis3>(lattice, expectation_value_function);

    // compute matrix
    const auto L = hubbard::make_liouvillian(J, U);
    blaze::CompressedMatrix<std::complex<double>, blaze::rowMajor> M;
    hubbard::init_matrix_no(L, M, basis, lattice, expectation_value_function);
    return 0;
